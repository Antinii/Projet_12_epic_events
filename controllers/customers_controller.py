...
def get_customers(session):
    ...
    for customer in customers:
        table.add_row(str(customer.id), customer.fullname, customer.email, customer.phone,
                      customer.company_name, str(customer.created_date),
                      str(customer.updated_date), customer.contact.name)

    console.print(table)


def create_customer(session, fullname, email, phone, company_name, contact_id):
    """
...
def update_customer(session, customer_id, fullname=None, email=None, phone=None, company_name=None):
    """
    ...
    customer.updated_date = datetime.now(timezone.utc)
    session.commit()
